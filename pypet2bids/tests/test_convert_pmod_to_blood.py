...
def Ex_bld_whole_blood_only_files():
    ...
    bld_files = [
        os.path.join(pmod_blood_dir, bld)
        for bld in os.listdir(pmod_blood_dir)
        if pathlib.Path(bld).suffix == ".bld"
    ]
...
def Ex_txt_manual_and_autosampled_mixed():
    ...
    bld_files = [
        os.path.join(pmod_blood_dir, bld)
        for bld in os.listdir(pmod_blood_dir)
        if pathlib.Path(bld).suffix == ".txt"
    ]
...
class TestPmodToBlood:
    def test_load_bld_files_blood_only(self, Ex_bld_whole_blood_only_files):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            pmod_to_blood = PmodToBlood(
                whole_blood_activity=pathlib.Path(
                    Ex_bld_whole_blood_only_files["whole"][0]
                ),
                plasma_activity=pathlib.Path(
    ...
    def test_load_bld_files_mixed(self, Ex_bld_manual_and_autosampled_mixed):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            pmod_to_blood = PmodToBlood(
                whole_blood_activity=pathlib.Path(
                    Ex_bld_manual_and_autosampled_mixed["whole"][0]
                ),
                plasma_activity=pathlib.Path(
                    Ex_bld_manual_and_autosampled_mixed["plasma"][0]
                ),
                parent_fraction=pathlib.Path(
    ...
    def test_bld_output_manual_popped_values(self, Ex_bld_manual_and_autosampled_mixed):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            created_files = [
                pathlib.Path(os.path.join(tempdir, created))
                for created in os.listdir(pathlib.Path(tempdir))
            ]
            assert (
                len(
                    [
                        ...
                        if "automatic" in created.name
                    ]
                )
                >= 1
            )
            assert (
                len([created for created in created_files if "manual" in created.name])
                >= 1
            )
    ...
    def test_load_txt_files_mixed(self, Ex_txt_manual_and_autosampled_mixed):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            pmod_to_blood = PmodToBlood(
                whole_blood_activity=pathlib.Path(
                    Ex_txt_manual_and_autosampled_mixed["whole"][0]
                ),
                plasma_activity=pathlib.Path(
                    Ex_txt_manual_and_autosampled_mixed["plasma"][0]
                ),
                parent_fraction=pathlib.Path(
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            for f in created_files:
                if "manual" in f.name and f.suffix == ".tsv":
                    ...
                    assert len(manual_df["plasma_radioactivity"]) == len(
                        manual_df["metabolite_parent_fraction"]
                    )

                if "auto" in f.name and f.suffix == ".tsv":
                    automatic_df = pandas.read_csv(f, sep="\t")
                    original_whole_blood = open_meta_data(
                        Ex_txt_manual_and_autosampled_mixed["whole"][0]
                    )
                    assert "whole_blood_radioactivity" in automatic_df.columns
                    assert len(automatic_df) + len(manual_df) == len(
                        original_whole_blood
                    )
