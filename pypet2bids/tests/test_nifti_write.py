...
if __name__ == "__main__":
    ...
    time_zero = datetime.fromtimestamp(
        read_and_write.ecat_header["DOSE_START_TIME"]
    ).strftime("%I:%M:%S")
    read_and_write.populate_sidecar()
    read_and_write.prune_sidecar()

    read_and_write.show_sidecar(
        os.path.join(
            os.path.dirname(nifti_path),
    ...
    read_from_ecat_but_not_written = ecat2nii(
        ecat_file=ecat_path, nifti_file=nifti_path, save_binary=True, sif_out=True
    )
