...
def test_cli_are_on_path():
    for cli in check_for_these_being_installed.keys():
        ...
        assert check_installed.returncode == 0, (
            f"{cli} should be installed and reachable via command line, this will"
...
def test_for_show_examples_argument():
    installed_cli = [
        key
        for key in check_for_these_being_installed.keys()
        if check_for_these_being_installed[key] is True
    ]
...
def test_kwargs_produce_valid_conversion(tmp_path):
    ...
    full_set_of_kwargs = {
        ...
        "ReconstructionParameterUnits": ["None", "None"],
        "ReconstructionParameterLabels": ["subsets", "iterations"],
        "ReconstructionParameterValues": [21, 3],
        ...
        "ConversionSoftware": ["dcm2niix", "pypet2bids"],
        "ConversionSoftwareVersion": ["v1.0.20211006", "0.0.8"],
        "TracerName": "[11C]PS13",
    ...
    dataset_description_path = (
        ecat_bids_dir.parent.parent.parent / "dataset_description.json"
    )
    ...
    dataset_description_path = (
        destination_path.parent.parent.parent / "dataset_description.json"
    )
    with open(dataset_description_path, "w") as outfile:
        json.dump(dataset_description_dictionary, outfile, indent=4)
    ...
    convert_dicom = subprocess.run(
        convert_dicom_command, shell=True, capture_output=True
    )

    dicom_cmd = (
        f"bids-validator {destination_path.parent.parent.parent} --ignoreWarnings"
    )
...
def test_spreadsheets_produce_valid_conversion_dcm2niix4pet(tmp_path):
    ...
    single_subject_spreadsheet = (
        PET2BIDS_DIR
        / "spreadsheet_conversion/single_subject_sheet/subject_metadata_example.xlsx"
    )

    dcm2niix4pet_test_dir = tmp_path / "dcm2niix_spreadsheet_input"
    dcm2niix4pet_test_dir.mkdir(parents=True, exist_ok=True)
    subject_folder = (
        dcm2niix4pet_test_dir / "sub-singlesubjectspreadsheetdicom" / "ses-test" / "pet"
    )
    ...
    validate_dicom_w_spreadsheet = subprocess.run(
        validator_cmd, shell=True, capture_output=True
    )
...
def test_spreadsheets_produce_valid_conversion_ecatpet2bids(tmp_path):
    ...
    subject_folder = (
        ecatpet2bids_test_dir / "sub-singlesubjectspreadsheetecat" / "ses-test" / "pet"
    )

    cmd = (
        f"python {ecatpet2bids} {ecat_file_path} "
    ...
    validate_ecat_w_spreadsheet = subprocess.run(
        validator_cmd, shell=True, capture_output=True
    )
...
def test_scanner_params_produce_valid_conversion(tmp_path):
    ...
    subject_folder = (
        ecatpet2bids_scanner_params_test_dir / "sub-01" / "ses-testecat" / "pet"
    )
    nifti_path = subject_folder / "sub-01_ses-testecat_pet.nii"

    # dump dataset description at destination path
    dataset_descripion_path = (
        ecatpet2bids_scanner_params_test_dir / "dataset_description.json"
    )
    ...
    ecat_command = (
        f"python {ecatpet2bids} {ecat_file_path} --scannerparams {scanner_params} --convert "
