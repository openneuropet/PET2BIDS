...
def create_sample_bytes(destination_file="python_sample_bytes.bytes"):
    ...
    with open(destination_file, "wb") as file:
        ...
        for b in bytes_to_write:
            print(
                f"Writing {b} to {destination_file}, byte order: {byte_order}, signed: {signed}"
            )
...
print(
    f"These are the arrays after being scaled by {calibration_factor}: {scaled_arrays}"
)
...
for k, v in scaled_arrays.items():
    ...
    print(
        f"loading that array results in the following: {nibabel.load(f'nibabel_{k}.nii.gz').get_fdata()}"
    )
...
for k, v in arrays_by_dtype.items():
    ...
    print(
        f"loading that array results in the following: {nibabel.load(f'nibabel_{k}_unscaled.nii.gz').get_fdata()}"
    )
