...
created. The PET JSON updater functions (updatejsonpetfile.m and update_json_pet_file.py) takes the existing Nifti sidecar JSON file and new metadata to add as input, checks that the full BIDS specification is respected (correct metadata but also consistency of metadata values for the different metadata keys) and updates the image sidecar JSON file. These functions are called by the dcm2niix4pet and ecat2nii (.m and .py) wrapper functions, but can just as easily be called directly allowing already converted image data to become BIDS compliant.
